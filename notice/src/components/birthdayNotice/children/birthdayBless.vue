<template>
  ...
  <div ref="scroll" class="blessing-page">
    <div class="blessing-container">
      <div class="head">
        <div class="head-img">
          <span class="icon" v-if="noticeItem.noticeDate == 0"><i class="icon-crown"></i></span>
          <img width="50" height="50" :src="noticeItem.avatar" alt="" v-if="noticeItem.avatar != null && noticeItem">
          <img v-else width="50" height="50" src="./logo.jpg">
        </div>
        <div class="user-name">
        ...
        <div class="solar-date">
          ...
          <span v-if="noticeItem.remindType == 0" class="user-date" id="userDate">{{noticeItem.solarDate | dateTime}}</span>
          <span v-else class="user-date" id="userDate">{{noticeItem.lunarDate}}</span>
          <p class="line"></p>
        </div>
        <div class="head-other" v-if="statusDate == 0">
          <span v-if="noticeDate == 0" class="text">今天是Ta的生日，快来送上祝福吧！</span>
          <span v-else class="text">{{noticeDate}}天后是Ta的生日，快来送上祝福吧！</span>
        </div>
        <div class="head-other" v-else>
          <span class="text">虽然Ta的生日已过，现在送祝福也未迟哦！</span>
          <!-- <span v-else class="text">{{noticeItem.noticetime}}天后是Ta的生日，快来送上祝福吧！</span> -->
        </div>
      </div>
      ...
      <div class="gift-wrapp">
        <h1 style="text-align: center; font-size: 14px; color: #fff">选礼物 送祝福</h1>
          ...
          <li class="other-li" v-for="(item, index) in noticeWishList">
            ...
            <div class="other-content">
              <div class="reply-user">
                ...
                <button v-else class="icon-comment" @click="commentOther(index, item.noticeWishId)"></button>
              </div>
                ...
                <li class="reply-list" v-for="(list, index) in item.caresResponseList">
                  ...
                  <div class="reply-content">
                    <h1 class="title">
                      ...
                      <button v-else class="icon-comment" @click="commentOther(index, list.wishResponseId)"></button>
                    </h1>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.statusDate = this.$route.query.statusDate
      this.noticeDate = this.$route.query.noticeDate
    ...
    methods: {
      ...
      async receiveBlessDatas() {
        ...
        const res = await http.post(url, api.serviceBirthday, params)
      ...
      onStopRecord() {
        ...
        // console.log(this.startTime)
        // console.log(stopTime-this.startTime)
        // 当按下和离开间隔时间太短时
        if (stopTime - this.startTime < 1000) {
          // 清零开始时间，且不录音
          this.startTime = 0
