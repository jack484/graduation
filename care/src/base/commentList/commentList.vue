    ...
    <div class="comment-list">
      <div class="input-box">
        <div class="textarea border-1px">
          <textarea rows="8" placeholder="说点什么吧..." v-model.trim="replyValue"></textarea>
        </div>
        <div class="confirm-box border-1px">
          <button class="qqface" @click="showFace">表情</button>
          <span class="input-num">{{inputNum}}</span>
          <div class="confirm">
            <button class="call-off" @click="hidComment">取消</button><!--
            <span v-if="responeId != null" class="yes" @click="onConfirmReply">发表</span> -->
            <button class="yes" @click.stop="onConfirm" :disabled="disabled">发表</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters([
        'getResponseId', 'getNoticeId', 'getSessionKey'
      ])
    },
    beforeDestroy(){ // 销毁定时器
    ...
    methods: {
      ...mapMutations([
        'DEL_RESPONSEID', 'DEL_NOTICEID'
      ]),
      fetchResponseId() {
        if (this.getResponseId) {
          this.wishResponseId = this.getResponseId
        }
      },
      fetchNoticeId() {
        if (this.getNoticeId) {
      ...
      async onConfirm() {
        ...
        if (!!this.noticeid) {
          params = {
            ...
            "wishResponseId": this.wishResponseId,
        ...
        const replayWish = await http.post(url, api.serviceReplyBless, params)
        // console.log(replayWish)
        if (replayWish.data.code === api.ERR_OK) {
          this.disabled = true
          Toast('发送成功')
          this.$emit('onConfirm')
    ...
    watch: {
      replyValue: function() {
        if (this.replyValue) {
          let num = this.replyValue.length
      ...
      getResponseId: function(nowId) {
        this.wishResponseId = nowId
        // console.log(this.wishResponseId)
      },
    }
  }
</script>
