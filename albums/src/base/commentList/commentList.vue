<template>
  ...
  <div class="comment-page">
      ...
      <div class="input-box">
        ...
        <div class="confirm-box border-1px">
          ...
          <div class="confirm">
            ...
            <button v-if="!getAlbumsReplyId" class="yes" @click="onConfirm" :disabled="disabled">发表</button>
            <button v-else class="yes" :disabled="disabled" @click="onConfirmReply">确认</button>
          </div>
        </div>
      </div>
    </div>
    <div class="face-wrapp" v-show="isShowFace">
      <ul class="face-ul">
        <li class="face-list" v-for="face in faceData" @touchend.stop.prevent="checkFace(face.name)">
          <img :src="face.imgUrl">
        </li>
      </ul>
    </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      getAlbumsReplyId: function(value) {
        this.replyId = value
      },
      getAlbumsId: function(value) {
        this.AlbumsId = value
      },
      getAlbumsDetailId: function(value) {
        this.AlbumsDetailId = value
      },
    },
    beforeDestroy(){ // 销毁定时器
    ...
    computed: {
      ...mapGetters([
        'getAlbumsReplyId', 'getAlbumsId', 'getAlbumsDetailId', 'getSessionKey'
      ])
    },
    mounted() {
      this.fetchId()
    ...
    methods: {
      ...mapMutations([
        'CLOSE_REPLYID', 'CLOSE_ALBUMSID', 'CLOSE_ALBUMSDETAILID'
      ]),
      fetchId() {
        if (this.getAlbumsReplyId) {
          this.replyId = this.getAlbumsReplyId
        }
        if (this.getAlbumsId) {
          this.AlbumsId = this.getAlbumsId
        }
        if (this.getAlbumsDetailId) {
          this.AlbumsDetailId = this.getAlbumsDetailId
        }
      },
      showFace() {
      ...
      async onConfirm() {
        ...
        const commentData = await http.post(url, api.serviceReply, params)
      ...
      async onConfirmReply() {
        ...
        let params = {
          "albumsId": this.AlbumsId,
          "albumsDetailId": this.AlbumsDetailId,
          "albumsResponseId": this.replyId,
          "replyContent": this.inputValue,
          "sessionKey": this.sessionKey,
        }
        this.disabled = !this.disabled
        let url = api.serviceUrl + this.getSessionKey
        const replyOtherData = await http.post(url, api.serviceAlbumsReply, params)
